...
const CAMPreviewPanel: React.FC<CAMPreviewPanelProps> = ({ applicationId, companyName = "Aether Dynamics Pvt Ltd" }) => {
    const previewData = [
        {
            title: "Character",
            icon: UserCheck,
            content: "Promoter group (Dr. Sharma) has 15+ years of high-precision manufacturing expertise with zero default history.",
            color: "blue"
        },
        {
            title: "Capacity",
    ...
    return (
        <motion.div
            ...
            whileInView="animate"
            viewport={{ once: true }}
            className="bg-white/80 backdrop-blur-xl rounded-[32px] border-2 border-emerald-500/20 shadow-[0_20px_50px_rgba(0,0,0,0.05)] overflow-hidden"
        >
            {/* Draft Header */}
            <div className="p-8 border-b border-slate-100 bg-gradient-to-r from-emerald-50/50 via-white to-blue-50/50">
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <h3 className="text-xl font-black text-[#1E293B] tracking-tight">CAM Generation Preview</h3>
                            <p className="text-[10px] font-bold text-emerald-600 uppercase tracking-[0.2em] mt-0.5">Automated Report Pre-submission Draft</p>
                        </div>
                    </div>
                    <div className="flex items-center gap-2 px-4 py-2 bg-white rounded-full border border-emerald-100 shadow-sm">
                        <div className="w-2 h-2 bg-emerald-500 rounded-full animate-pulse" />
                        <span className="text-[11px] font-black text-slate-700 uppercase tracking-widest">Ready for Export</span>
                    </div>
                </div>
            </div>
